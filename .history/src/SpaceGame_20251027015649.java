...
public class SpaceGame extends JPanel implements ActionListener, KeyListener {
    ...
    // Fire key states (player-triggered sound)
    private boolean p1FirePressed = false; // Space bar for player1
    private boolean p2FirePressed = false; // Right Control for player2 (co-op)
    ...
    if (coopMode && player2 != null) {
        ...
        if (System.currentTimeMillis() - player.getLastShotTime() > currentFireRate) {
            // If player pressed the fire key, play sound; otherwise auto-fire silently
            shootBullet(p1FirePressed);
        ...
        if (coopMode && player2 != null) {
            ...
            if (System.currentTimeMillis() - player2.getLastShotTime() > p2FireRate) {
                // Play sound only when player2 actively presses fire key
                shootBulletPlayer2(p2FirePressed);
    ...
    private void shootBullet(boolean playSound) {
        // Play bullet sound effect only if requested
        if (playSound) playBulletSound();
    ...
    private void shootBulletPlayer2(boolean playSound) {
        if (player2 == null) return;
    ...
    private void handleGameInput(int key, KeyEvent e) {
        ...
        // Space = Player1 fire button (active while held)
        ...
        // Right Control = Player2 fire button (co-op only)
        if (key == KeyEvent.VK_CONTROL && e.getKeyLocation() == KeyEvent.KEY_LOCATION_RIGHT && gameRunning && coopMode) {
            p2FirePressed = true;
        }
    ...
    public void keyReleased(KeyEvent e) {
        ...
        // Release fire keys
        if (key == KeyEvent.VK_SPACE) p1FirePressed = false;
        if (key == KeyEvent.VK_CONTROL && e.getKeyLocation() == KeyEvent.KEY_LOCATION_RIGHT) p2FirePressed = false;
    }
