...
public class SpaceGame extends JPanel implements ActionListener, KeyListener {
    ...
    // Menu Options
    private final String[] mainMenuOptions = {"Play Level 1", "Play Level 2 (Locked)", "Options", "Exit"};
    ...
    // Level1 Win Animation
    private boolean level1Unlocked = false; // Track if level 1 has been completed
    private BufferedImage[] winAnimationFrames; // Animation frames for boss explosion
    private int winAnimationIndex = 0;
    private long winAnimationStartTime = 0;
    private static final long WIN_ANIMATION_FRAME_DURATION = 300; // 300ms per frame
    private static final long WIN_ANIMATION_BLANK_DURATION = 200; // Blank screen duration
    ...
    private void loadWinAnimationFrames() {
        String[] animFrameNames = {
            "Ani/BOOM_0.png",
            "Ani/BOOM2_0.png",
            "Ani/BOOM3_0.png"
        };
        
        winAnimationFrames = new BufferedImage[animFrameNames.length];
        String[] basePaths = {
            "src/Pic/",
            "./src/Pic/",
            "Pic/",
        ...
        for (int i = 0; i < animFrameNames.length; i++) {
            String name = animFrameNames[i];
            ...
            try {
                java.io.InputStream is = getClass().getResourceAsStream("/Pic/" + name);
                if (is != null) {
                    winAnimationFrames[i] = ImageIO.read(is);
                    is.close();
                    loaded = true;
                    System.out.println("Loaded win animation frame from classpath: " + name);
                }
            } catch (IOException ex) {
                // continue
            }
            
            // Try filesystem paths if not loaded
            if (!loaded) {
                for (String base : basePaths) {
                    try {
                        File f = new File(base + name);
                        if (f.exists()) {
                            winAnimationFrames[i] = ImageIO.read(f);
                            System.out.println("Loaded win animation frame from: " + f.getAbsolutePath());
                            loaded = true;
                            break;
                        }
                    } catch (IOException ex) {
    ...
    if (coopMode && player2 != null) {
        ...
        if (bossDeathAnimationActive) {
            ...
            if (elapsed >= BOSS_DEATH_DURATION) {
                ...
                winAnimationIndex = 0;
                winAnimationStartTime = System.currentTimeMillis();
                level1Unlocked = true; // Unlock level 2
                stopBGM(); // Stop boss music
            }
        }
    ...
    String title = "Beyond the Red Eclipse: Exodus";
        ...
        for (int i = 0; i < mainMenuOptions.length; i++) {
            // Update Level 2 text based on unlock status
            String optionText = mainMenuOptions[i];
            if (i == 1) { // Level 2 option
                optionText = level1Unlocked ? "Play Level 2" : "Play Level 2 (Locked)";
            }
            
            if (i == selectedMenuOption) {
                // Dim locked options
                if (i == 1 && !level1Unlocked) {
                    g2d.setColor(Color.DARK_GRAY);
                } else {
                    g2d.setColor(Color.YELLOW);
                }
                g2d.drawString("> " + optionText + " <", (SCREEN_WIDTH - menuFm.stringWidth("> " + optionText + " <")) / 2, 280 + i * 50);
            } else {
                // Dim locked options
                if (i == 1 && !level1Unlocked) {
                    g2d.setColor(Color.DARK_GRAY);
    ...
    private void drawLevel1WinScreen(Graphics2D g2d) {
        long elapsed = System.currentTimeMillis() - winAnimationStartTime;
        
        // Calculate which frame to show
        // Blank -> BOOM_0 -> BOOM2_0 -> BOOM3_0
        long totalBlankTime = WIN_ANIMATION_BLANK_DURATION;
        long timePerFrame = WIN_ANIMATION_FRAME_DURATION;
        
        // Fill background black
        g2d.setColor(Color.BLACK);
        g2d.fillRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
        
        if (elapsed < totalBlankTime) {
            // Show blank screen
            // Do nothing, just black
        } else {
            // Show animation frames
            long animTime = elapsed - totalBlankTime;
            int frameIndex = (int) (animTime / timePerFrame);
            
            if (frameIndex < winAnimationFrames.length) {
                // Draw current animation frame centered
                BufferedImage frame = winAnimationFrames[frameIndex];
                if (frame != null) {
                    int x = (SCREEN_WIDTH - frame.getWidth()) / 2;
                    int y = (SCREEN_HEIGHT - frame.getHeight()) / 2;
                    g2d.drawImage(frame, x, y, null);
                }
            } else {
                // Animation finished, show victory text
                g2d.setColor(Color.CYAN);
                g2d.setFont(new Font("Arial", Font.BOLD, 48));
                FontMetrics titleFm = g2d.getFontMetrics();
                ...
                String msg1 = "LEVEL 1 COMPLETE!";
                String msg2 = "Level 2 Unlocked!";
                int msg1X = (SCREEN_WIDTH - msgFm.stringWidth(msg1)) / 2;
                int msg2X = (SCREEN_WIDTH - msgFm.stringWidth(msg2)) / 2;
                g2d.drawString(msg1, msg1X, SCREEN_HEIGHT / 2);
                g2d.drawString(msg2, msg2X, SCREEN_HEIGHT / 2 + 50);
                
                // Draw instruction
                g2d.setColor(Color.YELLOW);
                g2d.setFont(new Font("Arial", Font.PLAIN, 24));
                FontMetrics instructFm = g2d.getFontMetrics();
                String instruct = "Press ENTER to return to Main Menu";
                int instructX = (SCREEN_WIDTH - instructFm.stringWidth(instruct)) / 2;
                g2d.drawString(instruct, instructX, SCREEN_HEIGHT - 100);
            }
        }
    }
    ...
    private void handleMenuInput(int key) {
        ...
        if (key == KeyEvent.VK_UP) {
            ...
            switch (selectedMenuOption) {
                case 0: // Play Level 1 - go to spacecraft selection
                    currentState = GameState.SPACECRAFT_SELECT;
                    selectedSpacecraft = 0;
                    break;
                case 1: // Play Level 2
                    if (level1Unlocked) {
                        // TODO: Level 2 not implemented yet
                        System.out.println("Level 2 not implemented yet!");
                    } else {
                        // Level locked, do nothing or show message
                        System.out.println("Level 2 is locked! Complete Level 1 first.");
                    }
                    break;
                case 2: // Options
